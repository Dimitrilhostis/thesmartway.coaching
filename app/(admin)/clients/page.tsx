...
export default async function ClientsPage({
  ...
  const { data: clients } = await supabase
    .from('clients')
    .select('*, user:users!user_id(id, name, last_name, email)')
  ...
  const [{ data: program }, { data: note }, { data: messages }] = await Promise.all([
    ...
    supabase.from('messages').select('*, sender:users!sender_id(name)').or(`sender_id.eq.${selected?.user_id ?? 'none'},receiver_id.eq.${selected?.user_id ?? 'none'}`).order('sent_at', { ascending: true }).limit(50),
  ])
