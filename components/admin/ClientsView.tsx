...
export default function ClientsView({ clients, selected, activeId, program, note, messages, currentUserId }: Props) {
          ...
          return (
            <button key={client.id} onClick={() => selectClient(client.id)}
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium truncate">{client.user?.full_name ?? '—'}</p>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
              {selected ? (
                ...
                <p className="text-muted text-sm text-center mt-10">Sélectionne un client</p>
              )}
            </div>
          </div>
        )}
