...
export default function EspaceClient({
  ...
  return (
    <div className="flex min-h-[calc(100vh-56px)]">
      ...
      <aside className="hidden md:flex w-56 glass-dark border-r border-border p-4 flex-col shrink-0">
        <div className="flex items-center gap-2.5 mb-6 pb-4 border-b border-border">
          <div className="w-9 h-9 rounded-full bg-white/8 border border-border flex items-center justify-center text-xs font-medium text-cream shrink-0">
            {initials}
          </div>
          <div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm transition-all text-left ${
                  tab === item.id
                    ? 'bg-white/8 text-cream border border-accent/25'
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="glass shadow-glass-sm p-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {program.weeks[0].days.map(day => (
                          <div key={day.id} className="flex flex-col gap-2 py-2.5 border-b border-border last:border-0">
              ...
              {products.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {products.map(product => (
                    <div key={product.id} className="glass shadow-glass-sm overflow-hidden border border-border">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 glass-nav border-t border-border flex pb-safe z-30">
          {TABS.map(item => {
            const Icon = item.icon
            return (
              <button
